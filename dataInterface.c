...
/**
 * This function sets memory to it initial state.
 */
void memoryReset(){
	...
	for(int i=0; i<memory.size; i+=(cpu.word_width/8)){

			char address[50];
		        char content[50];
                        //obtain adrress in string format
		        wordHexadecimal(address, i);
...
void cacheDataReset(int level){
    ...
    contentArrayToString(cache_content, cache_content_char, level);

    //I write the all the reseted fields in each cache line
    for(int i=0; i<numLines; i++){ 
    
	wordHexadecimalN(cache_line, i, hexDigsLine);
	wordHexadecimalN(cache_tag, 0, hexDigsTag);
	wordHexadecimalN(cache_set, i/asociativity, hexDigsSet);
...
void showCacheLineData(int level, int i){
      ...
      contentArrayToString((line).content, contentString, level);
...
void writeLineCacheData(int level, struct cacheLine* line, int i){
	...
	wordHexadecimalN(c,(*line).set, hexDigsSet);
        d=(*line).user_content;
        f=(*line).valid;
	g=(*line).dirty;
 ...
 * @param i word index
 */
void showPosicionMemory(int i){

        struct memoryPosition pos;
        //I read the memory position
        readPosicionMemory(&pos, i);
        //I print the values
	printf("------------------------------------------------------\n");
 ...
 * @param i memory word index
 */
void readPosicionMemory(struct memoryPosition *pos, int i){
 ...
 * @param i memory word index
 */
void writePosicionMemory(struct memoryPosition *pos, int i){
	GtkTreeModel *model= GTK_TREE_MODEL(modelMEMORY);
	GtkTreeIter iter;
	char iterstring[50];
        sprintf(iterstring, "%d", i);	
	...
	char a[100];
        ...
        int digitsWord=ceil(log(cpu.word_width)/log(16));

	wordHexadecimal(a,(*pos).address);
	wordHexadecimal(b,(*pos).content);
	c=(*pos).user_content;
 ...
 */
void setColorMemoryPosition(int i, int color){
	GtkTreeModel *model= GTK_TREE_MODEL(modelMEMORY);
	GtkTreeIter iter;
	char iterstring[50];
        sprintf(iterstring, "%d", i);	
